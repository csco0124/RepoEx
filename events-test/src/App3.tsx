...
const App3 = (props: Props) => {
  ...
  const elemX = useRef<HTMLIFrameElement>(null);
  const elemY = useRef<HTMLIFrameElement>(null);

  const add = () => {
    console.log(elemX.current + ":" + elemX.current.value);
    console.log(`${elemY.current}:${elemY.current.value}`);
  ...
  return (
    <div className="container">
      ...
      <br />
      <button onClick={add}>덛셈 계산</button>
      <br />
      결과 : <span>{result}</span>
    </div>
  );
